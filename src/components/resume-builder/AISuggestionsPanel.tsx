...
export function AISuggestionsPanel({
  ...
  return (
    <div
      ...
      style={{ border: '1px solid rgba(124,58,237,0.2)', minHeight: 280 }}
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <Sparkles size={16} color="#a78bfa" />
          <span style={{ color: '#f8fafc', fontWeight: 700, fontSize: '0.88rem' }}>AI Suggestions</span>
        </div>
        <span style={{ color: '#64748b', fontSize: '0.72rem' }}>{pending.length} pending</span>
      </div>
      ...
      {pending.length > 0 && (
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={applyAll}
          className="mb-3 w-full py-2.5 rounded-xl flex items-center justify-center gap-2 text-sm font-bold"
          style={{ background: 'linear-gradient(135deg, rgba(124,58,237,0.35), rgba(6,182,212,0.2))', border: '1px solid rgba(124,58,237,0.4)', color: '#e9d5ff' }}
        >
          <Wand2 size={16} />
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-1" style={{ maxHeight: 'min(60vh, 520px)' }}>
        {suggestions.length === 0 && (
          <p style={{ color: '#64748b', fontSize: '0.8rem' }}>Run optimization to generate tailored suggestions.</p>
        )}
        {suggestions.map((s, i) => (
          <motion.div
            key={s.id}
          ...
          >
            <div className="flex items-start justify-between gap-2 mb-1">
              <div>
                <div style={{ color: '#e2e8f0', fontWeight: 600, fontSize: '0.8rem' }}>{s.title}</div>
                <div style={{ color: '#64748b', fontSize: '0.72rem', marginTop: 2 }}>{s.detail}</div>
              </div>
              <span className="shrink-0 text-[0.6rem] uppercase font-bold px-1.5 py-0.5 rounded" style={{ background: 'rgba(124,58,237,0.15)', color: '#c4b5fd' }}>
                {s.sectionId}
              </span>
            </div>
            {s.replacement && s.status === 'pending' && (
              <div className="mt-2 rounded-lg px-2 py-1.5 text-[0.75rem]" style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', color: '#6ee7b7' }}>
                {s.replacement}
              </div>
            )}
            {s.status === 'pending' && (
            ...
            {s.status !== 'pending' && (
              <div className="mt-2 text-[0.7rem] font-semibold" style={{ color: s.status === 'accepted' ? '#34d399' : '#94a3b8' }}>

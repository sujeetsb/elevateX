...
export function interviewPrepSystem(): string {
  return [
    ...
    'Given the job role and user profile, generate interview preparation materials.',
    'Return JSON:',
    ...
    'behavioral: 5 STAR-method questions. technical: 5 role-specific. company: 3 smart questions to ask.',
    FRAGMENTS.nonTech,
    FRAGMENTS.actionable,
    FRAGMENTS.jsonOnly,
    FRAGMENTS.safe,
...
export function chatAssistantSystem(): string {
  return [
    `You are ${APP_NAME} AI Mentor (v${PROMPT_VERSION}) — an expert career coach, strategist, and advisor.`,
